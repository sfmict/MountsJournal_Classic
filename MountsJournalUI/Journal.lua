...
function journal:gridModelSceneInit(btn, data, force)
	...
	local _,_, creatureID, descriptionText, sourceText, isSelfMount, mountType, modelSceneID, animID, spellVisualKitID, disablePlayerMountPreview = util.getMountInfoExtra(mountID)
...
function journal:updateMountDisplay(forceSceneChange, creatureID)
	...
	if self.selectedMountID then
		...
		if self.mountDisplay.lastMountID ~= self.selectedMountID or forceSceneChange then
			local _,_, creatureDisplayID, descriptionText, sourceText, isSelfMount, mountType, modelSceneID, animID, spellVisualKitID, disablePlayerMountPreview = util.getMountInfoExtra(self.selectedMountID)
			if not creatureID then
