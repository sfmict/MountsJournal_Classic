...
local C_MountJournal, C_PetJournal, wipe, tinsert, next, pairs, ipairs, select, type, sort, math, concat = C_MountJournal, C_PetJournal, wipe, tinsert, next, pairs, ipairs, select, type, sort, math, table.concat
...
function journal:init()
	...
	self.summonButton:HookScript("PreClick", function(btn)
		...
		if type(self.selectedMountID) == "number" then
			...
			if spellID == 5487 or spellID == 768 or spellID == 114282 or spellID == 210053
			or spellID == 783 or spellID == 33943 or spellID == 40120
			then
...
function journal:gridModelSceneInit(btn, data, force)
	...
	local _,_, creatureID, descriptionText, sourceText, isSelfMount, mountType, modelSceneID, animID, spellVisualKitID, disablePlayerMountPreview = self:getMountInfoExtra(mountID)
...
function journal:getFilterSpecific(spellID, isSelfMount, mountID)
