local addon, ns = ...
local config, journal, util = ns.config, ns.journal, ns.util
local iconData = CreateFrame("FRAME", nil, config, "MJDarkPanelTemplate")
...
local function fillOutExtraIconsWithMounts(extraIcons, icons)
	for i = 1, #journal.mountIDs do
		local name, spellID, icon = util.getMountInfo(journal.mountIDs[i])
...
iconData:SetScript("OnShow", function(self)
	...
	self.extraIcons = {}
	self.filtredIcons = {}
	fillOutExtraIconsWithMounts(self.extraIcons, self.icons)
	...
	sort(self.extraIcons, function(a, b)
		if a.type == "mount" and b.type ~= "mount" then return true
		elseif a.type ~= "mount" and b.type == "mount" then return false end
	...
	tinsert(self.extraIcons, 1, {type = "mount", name = SUMMON.." 1", icon = 303868})
	tinsert(self.extraIcons, 2, {type = "mount", name = SUMMON.." 2", icon = 237534})
	...
	self.view:SetElementInitializer("MJIconButtonSelectedTemplate", function(...) self:btnInit(...) end)
...
function iconData:filterDropDownInit(btn, level)
	...
	info.func = function(_,_,_, checked)
		self.filters.mount = checked
		self:refreshFilters()
	end
	info.checked = self.filters.mount
