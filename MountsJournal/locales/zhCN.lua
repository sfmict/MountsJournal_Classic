local _, ns = ...
local L = ns.L

L["author"] = "作者"
L["Main"] = "主要"
L["ConfigPanelTitle"] = "全局设置"
L["Class settings"] = "职业设置"
L["Modifier"] = "组合键"
L["Normal mount summon"] = "普通坐骑召唤."
L["SecondMountTooltipTitle"] = "如果组合键保持或\"%s 2\"被使用:"
L["SecondMountTooltipDescription"] = "如果你在水中，召唤非水下坐骑。\n\n如果你在陆地上并且你能飞，召唤地面坐骑。"
L["Drag to create a summon panel"] = "拖动创建召唤面板"
L["UseBindingTooltip"] = "使用按键绑定设置"
L["Summon panel"] = "召唤面板"
L["Left-button to drag"] = "左键拖拽"
L["Strata of panel"] = "面板层级"
L["Speed display"] = "速度显示"
L["Buttons"] = "按钮"
L["PANEL_UP"] = "上"
L["PANEL_DOWN"] = "下"
L["PANEL_LEFT"] = "左"
L["PANEL_RIGHT"] = "右"
L["Fade out (opacity)"] = "淡出（不透明度变化）"
L["Button size"] = "按钮尺寸"
L["Reset size"] = "重置尺寸"
L["Hide panel"] = "隐藏面板"
L["Target Mount"] = "目标坐骑"
L["Shows the mount of current target"] = "显示当前目标的坐骑"
L["LMB"] = "左键"
L["RMB"] = "右键"
L["Select mount"] = "选择坐骑"
L["Auto select Mount"] = "自动选择坐骑"
L["ZoneSettingsTooltip"] = "区域设置功能"
L["ZoneSettingsTooltipDescription"] = "为该区创建一个坐骑列表.\n\n配置区域标志.\n\n设置关联以在不同的区使用一个坐骑列表."
L["ButtonsSelectedTooltipDescription"] = "偏好不影响使用%s召唤坐骑。\n\n您应按类型选择坐骑，以便在适当的条件下召唤。"
L["ProfilesTooltipDescription"] = "配置设置, 配置管理着所选坐骑的列表, 区域和宠物的设置."
-- L["SettingsTooltipDescription"] = "Check settings, create macros or bind keys to use %s."
L["Handle a jump in water"] = "处理水中跳跃"
L["WaterJumpDescription"] = "在你跳入水中后, 将被召唤出非水下坐骑."
-- L["UseHerbMounts"] = "Use Mount for Herbalism"
-- L["UseHerbMountsDescription"] = "If Herbalism is learned, a suitable mount is used if available."
-- L["UseHerbMountsOnZones"] = "Only in Herb Gathering Zones"
-- L["Herb Gathering"] = "Herb Gathering"
-- L["HerbGatheringFlagDescription"] = "Used to configure use of Mount to Herbalism."
L["If item durability is less than"] = "如果物品耐久度低于"
L["In flyable zones"] = "在可飞行区域"
L["UseRepairMountsDescription"] = "如果至少有一件物品的耐久度低于指定的百分比, 所选坐骑将被召唤出来."
L["If the number of free slots in bags is less"] = "如果背包中的空格数量少于"
-- L["Random available mount"] = "Random available mount"
L["Use %s"] = "使用%s"
-- L["Use automatically"] = "Use automatically"
L["UseMagicBroomTitle"] = "使用魔法扫帚"
L["UseMagicBroomDescription"] = "当\"万圣节\"事件激活时, 如果你有\"魔法扫帚\", 它将被使用."
-- L["UseUnderlightAnglerDescription"] = "Use Underlight Angler instead of underwater mounts."
L["A macro named \"%s\" already exists, overwrite it?"] = "一个名为\"%s\"的宏已经存在，是否将其覆盖？"
L["CreateMacro"] = "创建宏"
L["CreateMacroTooltip"] = "创建的宏用于召唤选定的坐骑。"
L["or key bind"] = "或按键绑定"
L["ERR_MOUNT_NO_SELECTED"] = "你没有选择有效的坐骑。"
L["Collected:"] = "已收集"
L["Shown:"] = "显示:"
-- L["With multiple models"] = "With only multiple models"
L["hidden for character"] = "为角色隐藏"
L["only hidden"] = "仅隐藏"
L["Hidden by player"] = "被玩家隐藏"
-- L["Only new"] = "Only new"
L["types"] = "类型"
L["selected"] = "已选择"
L["SELECT_AS_TYPE_1"] = "选中为飞行坐骑"
L["SELECT_AS_TYPE_2"] = "选中为地面坐骑"
L["SELECT_AS_TYPE_3"] = "选中为水下坐骑"
L["MOUNT_TYPE_1"] = "飞行"
L["MOUNT_TYPE_2"] = "地面"
L["MOUNT_TYPE_3"] = "水下"
L["MOUNT_TYPE_4"] = "未选择"
L["Extra Actions"] = "额外动作"
L["Specific"] = "特定"
L["repair"] = "修复"
L["passenger"] = "乘客"
L["transform"] = "转换"
L["Multiple Models"] = "多种模式"
L["additional"] = "额外的"
L["rest"] = "其他"
L["factions"] = "阵营"
L["MOUNT_FACTION_1"] = "部落"
L["MOUNT_FACTION_2"] = "联盟"
L["MOUNT_FACTION_3"] = "通用"
L["sources"] = "来源"
L["PET_1"] = "与随机偏爱宠物"
L["PET_2"] = "与随机宠物"
L["PET_3"] = "与宠物"
L["PET_4"] = "不要宠物"
L["expansions"] = "资料片"
L["Tolerance"] = "容限"
L["Receipt date"] = "获得日期"
L["Travel time"] = "旅行时间"
L["Travel distance"] = "旅行距离"
L["Avg. speed"] = "平均速度"
L["Chance of summoning"] = "召唤几率"
L["Any"] = "任意"
L["ANY_GROUP"] = "任意"
L["> (more than)"] = "> (大于)"
L["< (less than)"] = "< (小于)"
L["= (equal to)"] = "= (等于)"
L["sorting"] = "排序"
L["Then Sort By"] = "然后排序方式"
L["Reverse Sort"] = "反向排序"
L["Collected First"] = "已收集优先"
L["Favorites First"] = "偏好优先"
L["Additional First"] = "额外优先"
L["Set current filters as default"] = "将当前的过滤设置为默认"
L["Restore default filters"] = "恢复默认过滤"
L["Enable Acceleration around the X-axis"] = "启用围绕X轴的加速功能"
L["Initial x-axis accseleration"] = "初始X轴加速"
L["X-axis accseleration"] = "X轴加速"
L["Minimum x-axis speed"] = "最小X轴速度"
L["Enable Acceleration around the Y-axis"] = "启用围绕Y轴的加速功能"
L["Initial y-axis accseleration"] = "初始Y轴加速"
L["Y-axis accseleration"] = "Y轴加速"
L["Minimum y-axis speed"] = "最小Y轴速度"
L["Model"] = "模型"
L["Map"] = "地图"
L["Map flags"] = "地图标记"
L["Settings"] = "设置"
L["Dungeons and Raids"] = "地下城和团队副本"
L["Current Location"] = "当前位置"
L["Enable Flags"] = "启用标记"
L["Ground Mounts Only"] = "仅地面坐骑"
L["Water Walking"] = "水上行走"
L["WaterWalkFlagDescription"] = "用来配置一些职业."
L["ListMountsFromZone"] = "使用区域中的坐骑列表"
L["No relation"] = "无关联"
L["Zones with list"] = "区域与列表"
L["Zones with relation"] = "区域与关联"
L["Zones with flags"] = "区域与标志"
L["CHARACTER_CLASS_DESCRIPTION"] = "（角色设置覆盖职业设置）"
L["HELP_MACRO_MOVE_FALL"] = "如果你在室内或正在移动，而你没有魔法扫帚或它被关闭，这个宏将被运行。"
L["HELP_MACRO_COMBAT"] = "如果你处于战斗状态，这个宏将被运行。"
L["CLASS_MOUNT_HINT"] = "%s 或 %s - 用于在战斗中召唤坐骑（不包括额外坐骑）"
L["CLASS_DMOUNT_HINT"] = "%s 或 %s - 用于解除坐骑"
L["CLASS_USEWHENCHARACTERFALLS"] = "当角色掉落时使用%s"
L["CLASS_USEWATERWALKINGSPELL"] = "召唤地面坐骑时使用%s"
L["CLASS_USEONLYWATERWALKLOCATION"] = "仅在水上行走区域使用"
L["PALADIN_USECRUSADERAURA"] = "召唤坐骑时使用%s"
L["PALADIN_RETURNLASTAURA"] = "解除坐骑时返回上一个光环"
L["DRUID_USELASTDRUIDFORM"] = "解除坐骑时返回之前形态"
L["DRUID_USETRAVELIFCANTFLY"] = "如果角色无法飞行，使用%s代替宏"
L["DRUID_USEDRUIDFORMSPECIALIZATION"] = "返回专精形态"
L["DRUID_USEMACROALWAYS"] = "使用这个宏来代替坐骑"
L["DRUID_USEMACROONLYCANFLY"] = "仅当角色可以飞行时"
L["Summonable Battle Pet"] = "可召唤的战斗宠物"
L["Summon Random Battle Pet"] = "召唤随机战斗宠物"
L["No Battle Pet"] = "无战斗宠物"
L["Summon a pet every"] = "召唤宠物每"
L["min"] = "分钟"
L["Summon only favorites"] = "仅召唤我喜欢的"
L["NoPetInRaid"] = "不要在团队中召唤战斗宠物"
L["NoPetInGroup"] = "不要在队伍中召唤战斗宠物"
L["Enable arrow buttons to browse mounts"] = "启用箭头按钮来浏览坐骑"
L["Show mount type selection buttons"] = "显示坐骑类型选择按钮"
L["CopyMountTarget"] = "尝试复制目标坐骑"
L["Open links in %s"] = "在%s中打开链接"
L["Click opens in"] = "点击打开到"
L["Show wowhead link in mount preview"] = "在坐骑预览中显示wowhead链接"
L["Enable statistics collection"] = "开启数据收集"
L["STATISTICS_DESCRIPTION"] = "收集坐骑的时间和距离（CPU负载，仅在角色乘坐坐骑时）"
L["Show mount in unit tooltip"] = "在单位鼠标提示中显示其坐骑"
L["Show Minimap Button"] = "显示小地图按钮"
L["Lock Minimap Button"] = "锁定小地图按钮"
L["|cffff7f3fClick|r to open %s"] = "|cffff7f3f点击|r打开%s"
L["Rule Set"] = "规则集"
L["Rule Sets"] = "规则集"
L["New rule set"] = "新建规则集"
L["A rule set with the same name exists."] = "同名的规则集已存在。"
L["Are you sure you want to delete rule set %s?"] = "你确定要删除规则集 %s 吗？"
L["Set as default"] = "设为默认"
L["Rule"] = "规则"
L["Rules"] = "规则"
L["RULES_TITLE"] = "坐骑规则。按顺序检查规则，第一个符合所有条件的规则将执行操作。"
L["Add Rule"] = "添加规则"
L["Add Group"] = "添加组"
L["Duplicate"] = "复制"
L["Import Rule"] = "导入规则"
L["Reset Rules"] = "重置规则"
L["Remove Rule %s"] = "删除规则 %s"
L["Alternative Mode"] = "替代模式"
L["NOT_CONDITION"] = "非"
L["Conditions"] = "条件"
L["Action"] = "动作"
L["Edit Rule"] = "编辑规则"
L["Edit Group"] = "编辑组"
L["ANY_MODIFIER"] = "任意"
L["Macro condition"] = "宏条件"
L["Mouse button"] = "鼠标按键"
L["Zone type"] = "区域类型"
L["Nameless holiday"] = "无名节日"
L["Flyable area"] = "可飞行区域"
L["Have item"] = "拥有物品"
L["Item is ready"] = "物品已就绪"
L["Item is equipped"] = "物品已装备"
L["Spell is known"] = "法术已学会"
L["Spell is ready"] = "法术已准备就绪"
L["Spell is usable"] = "法术可用"
L["Zone Name/Subzone Name"] = "区域名/子区域名"
L["The player has a buff"] = "玩家有增益效果"
L["The player has a debuff"] = "玩家有减益效果"
L["The player is falling"] = "玩家正在坠落"
L["The player is moving"] = "玩家正在移动"
L["The player is indoors"] = "玩家在室内"
L["The player is swimming"] = "玩家正在游泳"
L["The player is mounted"] = "玩家在坐骑上"
L["The player is within an vehicle"] = "玩家在载具内"
L["The player is dead"] = "玩家已死亡"
L["The player is resting"] = "玩家处于休息状态"
L["The player is in combat"] = "玩家处于战斗状态"
L["Sex"] = "性别"
L["Group Type"] = "队伍类型"
L["Not Found"] = "未找到"
L["Friend in Party"] = "队伍中有好友"
L["Friend in Raid"] = "团队中有好友"
L["Get State"] = "获取状态"
L["Get a state that can be set in actions using \"Set State\""] = "使用“设置状态”获取可在操作中设置的状态"
L["Set State"] = "设置状态"
L["Set a state that can be read in conditions using \"Get State\""] = "设置可通过 “获取状态 ”读取的状态"
L["Doesn't interrupt the rule queue"] = "不中断规则队列"
L["Random Mount"] = "随机坐骑"
L["Random Mount of Selected Type"] = "随机选定类型坐骑"
L["Random Mount by Summon Counter"] = "按召唤计数器随机坐骑"
L["Random Mount of Selected Type by Summon Counter"] = "按召唤计数器随机选定类型坐骑"
L["The lower the counter, the higher the chance"] = "计数器越低，几率越高"
L["Selected profile"] = "已选配置文件"
L["Mount"] = "坐骑"
L["TMOUNT_DESCRIPTION"] = "失败时不中断规则执行"
L["Use Item"] = "使用物品"
L["Use Inventory Item"] = "使用库存物品"
L["Cast Spell"] = "施放法术"
L["Use macro before mounting"] = "上坐骑前使用宏"
L["PMACRO_DESCRIPTION"] = "注册上坐骑前要使用的宏"
L["Snippet"] = "代码片段"
L["Code Snippet"] = "代码片段"
L["Code Snippets"] = "代码片段"
L["Add Snippet"] = "添加代码片段"
L["Import Snippet"] = "导入代码片段"
L["A snippet with the same name exists."] = "同名的代码片段已存在。"
L["Are you sure you want to delete snippet %s?"] = "你确定要删除代码片段 %s 吗？"
L["Line"] = "行号"
L["Examples"] = "示例"
L["Tab Size"] = "制表符大小"
L["Do you want to save changes?"] = "是否保存更改？"
L["About"] = "关于"
L["Help with translation of %s. Thanks."] = "参与帮助翻译 %s，谢谢！"
L["Localization Translators:"] = "本地化翻译者:"
L["ABBR_YARD"] = "码"
L["ABBR_MILE"] = "英里"
L["ABBR_METER"] = "米"
L["ABBR_KILOMETER"] = "千米"
L["ABBR_HOUR"] = "时"
L["Right-click for more options"] = "右键显示更多选项"
L["Shift-click to create a chat link"] = "Shift-点击 创建聊天链接"
L["Requesting data from %s ..."] = "正在从 %s 请求数据..."
L["Error not receiving data from %s ..."] = "从 %s 接收数据时出错..."
L["Malformed link"] = "格式错误的链接"
L["Transmission error"] = "传输错误"
L["Receiving data from %s"] = "正在从 %s 接收数据"
L["Received from"] = "接收自"
-- ANIMATIONS
L["Default"] = "默认"
L["Mount special"] = "坐骑特效"
L["Walk"] = "走"
L["Walk backwards"] = "向后走"
L["Run"] = "跑"
L["Swim idle"] = "游泳闲置"
L["Swim"] = "游泳"
L["Swim backwards"] = "向后游"
L["Fly stand"] = "飞行状态"
L["Fly"] = "飞行"
L["Fly backwards"] = "向后飞"
L["Loop"] = "循环"
L["Are you sure you want to delete animation %s?"] = "你确定要删除动画\"%s\"吗？"
-- PROFILES
L["Profile"] = "配置文件"
L["Profiles"] = "配置"
L["New profile"] = "新配置"
L["Create"] = "创建"
L["Copy current"] = "复制当前"
L["Export"] = "导出"
L["Import"] = "导入"
L["A profile with the same name exists."] = "同名的配置已存在。"
L["HOLIDAY_ACTIVE"] = "激活"
L["Profile settings"] = "配置设置"
L["Pet binding from default profile"] = "默认配置下的宠物绑定"
L["Zones settings from default profile"] = "默认配置中的区域设置"
L["Auto add new mounts to selected"] = "自动添加新坐骑到已选择"
L["Select all filtered mounts"] = "选择所有过滤后的坐骑"
L["Unselect all filtered mounts"] = "取消选择所有过滤后的坐骑"
L["Select all favorite mounts"] = "选择所有喜爱的坐骑"
L["Unselect all favorite mounts"] = "取消选择所有喜爱的坐骑"
L["Select all mounts"] = "选择所有坐骑"
L["Unselect all mounts"] = "取消选择所有坐骑"
L["Are you sure you want to delete profile %s?"] = "你确定要删除配置\"%s\"吗？"
L["Are you sure you want %s?"] = "你确定你想要\"%s\"吗？"
-- TAGS
L["tags"] = "标签"
L["No tag"] = "无标签"
L["With all tags"] = "包含所有标签"
L["Add tag"] = "添加标签"
L["Tag already exists."] = "标签已经存在."
L["Are you sure you want to delete tag %s?"] = "你确定要删除标签\"%s\"吗？"
-- FAMILY
L["Family"] = "生物科"
L["Airplanes"] = "飞机"
L["Airships"] = "飞艇"
L["Albatross"] = "信天翁"
L["Alpacas"] = "羊驼"
L["Amphibian"] = "两栖"
L["Animite"] = "心蛛"
L["Aqir Flyers"] = "工蜂"
L["Arachnids"] = "蛛形"
L["Armoredon"] = "厚甲龙"
L["Assault Wagons"] = "攻城车"
L["Basilisks"] = "蜥蜴"
L["Bats"] = "蝙蝠"
L["Bears"] = "熊"
L["Beetle"] = "甲虫"
L["Bipedal Cat"] = "梦爪獍"
L["Birds"] = "鸟类"
L["Blood Ticks"] = "血虱"
L["Boars"] = "野猪"
L["Book"] = "书"
L["Bovids"] = "牛科"
L["Broom"] = "扫帚"
L["Brutosaurs"] = "雷龙"
L["Camels"] = "骆驼"
L["Carnivorans"] = "食肉"
L["Carpets"] = "飞毯"
L["Cats"] = "猫科"
L["Cervid"] = "元鹿"
L["Chargers"] = "战马"
L["Chickens"] = "鸡"
L["Clefthooves"] = "裂蹄牛"
L["Cloud Serpents"] = "云端翔龙"
L["Core Hounds"] = "熔火犬"
L["Crabs"] = "螃蟹"
L["Cranes"] = "仙鹤"
L["Crawgs"] = "抱齿兽"
L["Crocolisks"] = "鳄鱼"
L["Crows"] = "乌鸦"
L["Demonic Hounds"] = "魔能猎犬"
L["Demonic Steeds"] = "恶魔马"
L["Demons"] = "恶魔"
L["Devourer"] = "吞噬者"
L["Dinosaurs"] = "恐龙"
L["Dire Wolves"] = "恐狼"
L["Direhorns"] = "恐角龙"
L["Discs"] = "飞碟"
L["Dragonhawks"] = "龙鹰"
L["Drakes"] = "龙"
L["Dread Ravens"] = "恐惧渡鸦"
L["Dreamsaber"] = "梦刃豹"
L["Eagle"] = "雄鹰"
L["Elekks"] = "雷象"
L["Elemental Hawks"] = "元素猎鹰"
L["Elementals"] = "元素生物"
L["Falcosaurs"] = "隼龙"
L["Fathom Rays"] = "海波鳐"
L["Feathermanes"] = "羽鬃兽"
L["Felsabers"] = "邪刃豹"
L["Fire Hawks"] = "火鹰"
L["Fish"] = "鱼类"
L["Flies"] = "苍蝇"
L["Flying Steeds"] = "天马"
L["Foxes"] = "狐"
L["Gargon"] = "加尔贡"
L["Gargoyle"] = "石裔魔"
L["Goats"] = "山羊"
L["Gorger"] = "饕餮者"
L["Gorm"] = "戈姆"
L["Grand Drakes"] = "大型龙"
L["Gronnlings"] = "小戈隆"
L["Grovegliders"] = "林间滑翔者"
L["Gryphons"] = "狮鹫"
L["Gyrocopters"] = "旋翼机"
L["Hands"] = "手"
L["Hawkstriders"] = "陆行鸟"
L["Hippogryphs"] = "角鹰兽"
L["Horned Steeds"] = "角马"
L["Horses"] = "马"
L["Hounds"] = "犬"
L["Hovercraft"] = "气垫船"
L["Humanoids"] = "人型生物"
L["Hyenas"] = "土狼"
L["Infernals"] = "地狱火"
L["Insects"] = "昆虫"
L["Jellyfish"] = "水母"
L["Jet Aerial Units"] = "空中单位"
L["Kites"] = "风筝"
L["Kodos"] = "科多兽"
L["Krolusks"] = "三叶虫"
L["Larion"] = "翼狮"
L["Lions"] = "狮"
L["Lizards"] = "蝾螈"
L["Lupine"] = "元狼"
L["Mammoths"] = "猛犸象"
L["Mana Rays"] = "法力鳐"
L["Manasabers"] = "魔刃豹"
L["Mauler"] = "重殴者"
L["Mechanical Animals"] = "机械动物"
L["Mechanical Birds"] = "机械鸟"
L["Mechanical Cats"] = "机械猫"
L["Mechanical Steeds"] = "机械马"
L["Mechanostriders"] = "机械陆行鸟"
L["Mecha-suits"] = "机甲"
L["Meeksi"] = "米克西"
L["Mollusc"] = "软体动物"
L["Moose"] = "驼鹿"
L["Moth"] = "蛾"
L["Motorcycles"] = "摩托车"
L["Mountain Horses"] = "高山马"
L["Murloc"] = "鱼人"
L["Mushan"] = "穆山兽"
L["Nether Drakes"] = "灵翼幼龙"
L["Nether Rays"] = "虚空鳐"
L["N'Zoth Serpents"] = "恩佐斯蛇"
L["Others"] = "其他"
L["Ottuk"] = "奥獭"
L["Owl"] = "猫头鹰"
L["Owlbear"] = "月兽"
L["Ox"] = "公牛"
L["Pandaren Phoenixes"] = "熊猫人凤凰"
L["Parrots"] = "鹦鹉"
L["Peafowl"] = "孔雀"
L["Phoenixes"] = "凤凰"
L["Proto-Drakes"] = "始祖幼龙"
L["Pterrordaxes"] = "翼手龙"
L["Quilen"] = "魁麟"
L["Rabbit"] = "兔子"
L["Rams"] = "公羊"
L["Raptora"] = "元鹰"
L["Raptors"] = "迅猛龙"
L["Rats"] = "鼠"
L["Rays"] = "鳐鱼"
L["Razorwing"] = "刀翼兽"
L["Reptiles"] = "爬行动物"
L["Rhinos"] = "犀牛"
L["Riverbeasts"] = "淡水兽"
L["Roc"] = "大鹏"
L["Rockets"] = "火箭"
L["Rodent"] = "啮齿动物"
L["Ruinstriders"] = "废墟游荡者"
L["Rylaks"] = "双头飞龙"
L["Sabers"] = "刃豹"
L["Scorpions"] = "蝎子"
L["Sea Serpents"] = "海蛇"
L["Seahorses"] = "海马"
L["Seat"] = "摇篮"
L["Shardhides"] = "碎皮兽"
L["Silithids"] = "异种蝎"
L["Slitherdrakes"] = "滑翔龙"
L["Slug"] = "蛞蝓"
L["Snail"] = "蜗牛"
L["Snapdragons"] = "毒鳍龙"
L["Spider Tanks"] = "蜘蛛坦克"
L["Spiders"] = "蜘蛛"
L["Sporebat"] = "孢子蝠"
L["Stag"] = "牡鹿"
L["Steeds"] = "马"
L["Stingrays"] = "鳐鱼"
L["Stone Cats"] = "石猎豹"
L["Stone Drakes"] = "石幼龙"
L["Surfboard"] = "冲浪板"
L["Talbuks"] = "塔布羊"
L["Tallstriders"] = "长腿鸟"
L["Talonbirds"] = "鸦神"
L["Tauralus"] = "荒牛"
L["Thunder Lizard"] = "雷霆蜥蜴"
L["Tigers"] = "虎"
L["Toads"] = "蟾蜍"
L["Turtles"] = "龟"
L["Undead Drakes"] = "亡灵幼龙"
L["Undead Steeds"] = "亡灵战马"
L["Undead Wolves"] = "亡灵战狼"
L["Ungulates"] = "有蹄"
L["Ur'zul"] = "乌祖尔"
L["Vehicles"] = "载具"
L["Vombata"] = "元袋熊"
L["Vulpin"] = "烁裘"
L["Vultures"] = "秃鹫"
L["War Wolves"] = "战狼"
L["Wasp"] = "巨蜂"
L["Water Striders"] = "水黾"
L["Wilderlings"] = "荒蚺"
L["Wind Drakes"] = "风幼龙"
L["Wolfhawks"] = "狼鹰"
L["Wolves"] = "狼"
L["Wyverns"] = "双足飞龙"
L["Yaks"] = "牦牛"
L["Yetis"] = "雪人"
