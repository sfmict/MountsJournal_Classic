local _, ns = ...
local type, concat, strconcat = type, table.concat, strconcat
local UnitClass, UnitRace, GetNumSpecializations, GetSpecializationInfo = UnitClass, UnitRace, GetNumSpecializations, C_SpecializationInfo.GetSpecializationInfo
...
function conds.tmog:getFuncText(values, addKey, _, ...)
	...
	if type(values) == "table" then
		...
		elseif num == 1 then vals = vals[1]
			...
			local var = ("_"):join("var", ...)
			addKey(strconcat("local ", var, " = {['", concat(vals, "']=true,['"), "']=true}"))
			return ("self:anyTtransmogOutfitActive(%s)"):format(var)
		end
	else
		local name, guid = ("\n"):split(values, 2)
